...
def test_physics_track_limitation_is_disclosed():
    """The physics cross-check is conditioned on the model's chosen class."""
    source = AUTO_FORECAST.read_text(encoding='utf-8')
    # Sanity: the code really does branch on the model's prediction.
    assert re.search(r"if hazard == 'Tropical Cyclone'", source), (
        'auto_forecast.py no longer conditions physics severity on the predicted class — '
        'if this was fixed, update docs/MODEL_CARD.md §6/§8 and this test together'
    )
    copy = _copy_text()
    assert re.search(r'(model selected|model has missed|class the model)', copy, re.I), (
        'public copy must disclose that the physics track is conditioned on the model\'s '
        'hazard choice and cannot independently detect a missed hazard '
        '(docs/PRODUCT_SPEC.md §5.4)'
    )
