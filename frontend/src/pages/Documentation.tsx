...
export const Documentation: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Title Banner */}
      <div className="bg-white border border-carbon-20 rounded-2xl p-6 md:p-8 shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-4">
          <div className="px-4 py-2 rounded-xl bg-carbon-10 text-carbon-60 text-xs font-bold border border-carbon-20">
            Hosted inference keeps model weights private
          </div>
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <Link
              to="/download?selectedTab=python"
              className="px-4 py-2 rounded-xl bg-carbon-10 hover:bg-carbon-20 text-carbon-80 text-xs font-bold transition-all border border-carbon-20 flex items-center gap-1.5"
            >
          ...
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <a
              ...
              className="px-4 py-2 rounded-xl bg-carbon-10 text-carbon-80 text-xs font-bold border border-carbon-20 hover:bg-carbon-20 transition-all flex items-center gap-1.5"
            >
              <span>⭐</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono text-xs">
        <motion.div whileHover={{ y: -3 }} className="bg-white border border-carbon-20 p-5 rounded-2xl space-y-1 shadow-xs">
          <span className="text-[10px] uppercase text-carbon-60 block font-bold">Input Tensor Shape</span>
          <span className="text-lg font-bold text-carbon-90">(1, 15, 10, 64, 64)</span>
          <p className="text-[11px] text-carbon-60 font-sans mt-1">NCDHW (Batch=1, Channels=15, Timesteps=10, Height=64, Width=64)</p>
        </motion.div>
        <motion.div whileHover={{ y: -3 }} className="bg-white border border-carbon-20 p-5 rounded-2xl space-y-1 shadow-xs">
          <span className="text-[10px] uppercase text-carbon-60 block font-bold">Format & Precision</span>
          <span className="text-lg font-bold text-carbon-90">FP32 TFLite</span>
          <p className="text-[11px] text-carbon-60 font-sans mt-1">Bypassed INT8 quantization due to CONV_3D operator constraints</p>
        </motion.div>
        <motion.div whileHover={{ y: -3 }} className="bg-white border border-carbon-20 p-5 rounded-2xl space-y-1 shadow-xs">
          <span className="text-[10px] uppercase text-carbon-60 block font-bold">Dual Output Heads</span>
          <span className="text-lg font-bold text-carbon-90">Softmax + Sigmoid</span>
          <p className="text-[11px] text-carbon-60 font-sans mt-1">8 Hazard Classes + Continuous Physical Severity Index [0.0 - 1.0]</p>
        </motion.div>
      </div>

      {/* 15 Multispectral Channels Grid */}
      <div className="bg-white border border-carbon-20 rounded-2xl p-6 space-y-4 shadow-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
          {[
            ...
            <motion.div whileHover={{ scale: 1.01, y: -2 }} key={item.ch} className="bg-carbon-05 border border-carbon-20 p-3 rounded-xl space-y-1">
              <div className="flex items-center justify-between">
                <span className="font-mono font-bold text-carbon-90">Channel {item.ch}</span>
                <span className="text-[10px] text-carbon-60 font-mono">{item.source}</span>
              </div>
              <div className="font-bold text-carbon-80 text-xs">{item.name}</div>
              <p className="text-[11px] text-carbon-60">{item.usage}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-carbon-20 rounded-2xl p-6 space-y-3 shadow-xs">
        <h2 className="text-lg font-bold text-carbon-90 flex items-center gap-2">
          <MaterialIcon name="functions" className="text-xl text-amber-600" />
          <span>Dual-Head Loss Function Formulation</span>
        </h2>
        <div className="p-4 bg-carbon-90 rounded-xl font-mono text-xs text-amber-400 border border-carbon-80 overflow-x-auto font-bold">
      ...
      <div className="bg-carbon-90 text-white rounded-2xl p-6 space-y-2 shadow-md">
      ...
      {/* Quick Action Footer */}
      <div className="bg-carbon-90 text-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-md">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <Link
              to="/contact?form=api"
              className="px-4 py-2.5 rounded-xl bg-nasa-red text-carbon-90 font-bold text-xs hover:bg-nasa-red-shade transition-all shadow-xs"
            >
              🔑 Request API Key
            </Link>
          </motion.div>
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <Link
              to="/use-cases"
              className="px-4 py-2.5 rounded-xl bg-carbon-80 text-white font-bold text-xs hover:bg-carbon-70 transition-all border border-carbon-70"
            >
              🌾 View Use Cases
            </Link>
          </motion.div>
