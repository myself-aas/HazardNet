...
export const DisasterDetailModalUI: React.FC<DisasterDetailModalUIProps> = ({
  ...
  const renderReportBody = () => (
    <>
      ...
      <div className="print-only mb-4 p-4 bg-white border-2 border-carbon-90 rounded-xl space-y-3">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 text-[8pt] font-mono text-carbon-60">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-carbon-05 border border-carbon-20 p-3.5 rounded-xl space-y-1">
          ...
          <div className="text-sm sm:text-base font-extrabold text-rose-600 font-mono">
            {data.estimatedImpactAreaKm2.toLocaleString()} <span className="text-xs font-normal text-carbon-60">sq km</span>
          </div>
          <div className="w-full bg-carbon-20 h-1.5 rounded-full overflow-hidden mt-1">
            <div
      ...
      <div className="bg-carbon-05 border border-carbon-20 p-4 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div>
            <h4 className="text-xs font-mono uppercase tracking-wider text-carbon-60">
              Live Hydro-Meteorological Telemetry
            </h4>
            <p className="text-xs font-bold text-carbon-90 mt-0.5">
      ...
      <div className="screen-only border-b border-carbon-20 flex items-center gap-2 overflow-x-auto text-xs py-1">
        <button
      ...
      <div className="screen-only">
        {activeTab === 'upazilas' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-carbon-60 font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {data.impactedUpazilas.map((up, idx) => (
                <div
                  key={idx}
                  className="p-3 bg-carbon-05 border border-carbon-20 rounded-xl flex items-center justify-between gap-3 text-xs"
                >
                  <div>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-carbon-05 border border-carbon-20 rounded-xl space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <div className="w-full bg-carbon-20 h-3 rounded-full overflow-hidden p-0.5 border border-carbon-30">
                  <div
                ...
                {data.modelAssessment.confidenceProbabilities.map((prob, idx) => (
                  <div key={idx} className="space-y-1 text-xs">
                    <div className="flex justify-between font-mono">
                      ...
                      <span className="text-carbon-90 font-bold">{(prob.probability * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full bg-carbon-20 h-2 rounded-full overflow-hidden border border-carbon-30">
                      <div
                        className={`h-full rounded-full ${
                          idx === 0 ? 'bg-purple-600' : idx === 1 ? 'bg-blue-500' : 'bg-carbon-50'
                        }`}
        ...
        {activeTab === 'emergency' && (
          <div className="space-y-4">
            ...
            <div className="p-4 bg-carbon-05 border border-carbon-20 rounded-xl space-y-2.5">
              <h4 className="text-xs font-bold text-carbon-90 uppercase tracking-wider flex items-center gap-2">
        ...
        {activeTab === 'history' && (
          <div className="p-4 bg-carbon-05 border border-carbon-20 rounded-xl space-y-3 text-xs">
            <h4 className="font-extrabold text-carbon-90 text-sm flex items-center gap-2">
              <span>Historical Disaster Benchmark Analysis</span>
            </h4>
            <p className="text-carbon-70 leading-relaxed bg-white p-3.5 rounded-lg border border-carbon-20">
      ...
      <div className="print-only space-y-5">
          ...
          <table className="w-full text-left text-xs border border-carbon-30">
            <thead>
              <tr className="bg-carbon-10 text-[8pt] font-mono">
                ...
                <th className="p-2 border border-carbon-30">Inundation Status</th>
                <th className="p-2 border border-carbon-30">Households Affected</th>
                <th className="p-2 border border-carbon-30">Severity Index</th>
              </tr>
            </thead>
            <tbody>
              {data.impactedUpazilas.map((up, idx) => (
                <tr key={idx} className="border-b border-carbon-20">
                  <td className="p-2 font-bold text-carbon-90 border border-carbon-20">{up.name}</td>
                  <td className="p-2 font-semibold text-carbon-80 border border-carbon-20">{up.status}</td>
                  <td className="p-2 font-mono text-carbon-70 border border-carbon-20">{up.householdsAffected.toLocaleString()}</td>
                  <td className="p-2 font-mono font-bold text-carbon-90 border border-carbon-20">{(up.severityScore * 100).toFixed(0)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs bg-carbon-05 p-3 rounded-lg border border-carbon-20 font-mono">
            <div>
              <span className="text-carbon-60 block text-[9pt]">Continuous Severity Index:</span>
            ...
            <div>
              <span className="text-carbon-60 block text-[9pt]">Predicted Hazard Type:</span>
              <strong className="text-sm font-black text-carbon-90">{data.hazardSubtype}</strong>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2 text-xs font-mono mb-2">
            <div className="p-2 bg-carbon-05 border border-carbon-30 rounded">
              <span className="text-carbon-60 text-[8pt] block">Active Shelters</span>
              <strong className="text-carbon-90">{data.emergencyResponse.activeShelters} Units</strong>
            </div>
            <div className="p-2 bg-carbon-05 border border-carbon-30 rounded">
              <span className="text-carbon-60 text-[8pt] block">Capacity Used</span>
              <strong className="text-carbon-90">{data.emergencyResponse.shelterCapacityUsedPercent}%</strong>
            </div>
            <div className="p-2 bg-carbon-05 border border-carbon-30 rounded">
              <span className="text-carbon-60 text-[8pt] block">Relief Dispatched</span>
              <strong className="text-carbon-90">{data.emergencyResponse.reliefDistributedTons} Tons</strong>
            </div>
            <div className="p-2 bg-carbon-05 border border-carbon-30 rounded">
              <span className="text-carbon-60 text-[8pt] block">Medical Teams</span>
              <strong className="text-carbon-90">{data.emergencyResponse.medicalTeamsDeployed} Teams</strong>
            </div>
          </div>

          <div className="bg-carbon-05 p-3 rounded-lg border border-carbon-30">
            <div className="font-bold text-xs uppercase mb-1 text-carbon-90">Immediate Standing Directives:</div>
            <ul className="space-y-1 text-xs text-carbon-80">
              {data.emergencyResponse.advisoryBullets.map((bullet, idx) => (
                <li key={idx} className="flex items-start gap-1.5">
                  <span className="font-bold text-carbon-90">•</span>
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-carbon-80 leading-relaxed bg-carbon-05 p-3 rounded-lg border border-carbon-20">
            {data.historicalComparison}
          </p>
        </div>
      </div>
      ...
      {isOpen && data && (
        <>
          ...
          <div className="p-4 sm:p-5 bg-carbon-05 border-b border-carbon-20 flex items-start justify-between gap-4">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-xl bg-white hover:bg-carbon-10 text-carbon-60 hover:text-carbon-90 border border-carbon-20 transition-all text-xs font-black shadow-xs min-h-[40px] px-3 flex items-center gap-1.5 cursor-pointer"
          ...
          {/* Action Bar Footer */}
          <div className="p-4 bg-carbon-05 border-t border-carbon-20 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              <PdfExportButton
            ...
            <button
              onClick={onClose}
              className="px-5 py-2 bg-white hover:bg-carbon-10 text-carbon-80 font-bold rounded-xl border border-carbon-20 transition-all shadow-xs min-h-[44px]"
            >
              Close Dialog
            </button>
          </div>
      ...
      <div className="fixed inset-0 z-[1200] sm:hidden flex flex-col justify-end pointer-events-none">
        ...
        >
          ...
          <div
            ...
            className="pt-2.5 pb-2 px-4 bg-carbon-05 border-b border-carbon-20 cursor-grab active:cursor-grabbing select-none flex flex-col items-center shrink-0"
          >
          ...
          {sheetMode === 'peek' ? (
            <div className="p-3 bg-white flex flex-col gap-2">
              <div className="flex items-center justify-between text-xs font-semibold text-carbon-70">
                <span>Hazard: <strong className="text-carbon-90">{data.hazardSubtype}</strong></span>
              ...
              <div className="w-full bg-carbon-10 h-2 rounded-full overflow-hidden border border-carbon-20">
                <div
                  className={`h-full rounded-full ${
                    data.modelAssessment.continuousSeverityIndex < 0.33
                      ? 'bg-emerald-500'
            ...
            <div className="p-4 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
              ...
              <div className="pt-3 border-t border-carbon-20 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={onClose}
                    className="px-4 py-3 bg-carbon-10 active:bg-carbon-20 text-carbon-70 font-bold rounded-xl border border-carbon-20 text-xs transition-all min-h-[44px]"
                  >
                    Close Sheet
                  </button>
                </div>
