...
export const Card: React.FC<{
  ...
  <section className={`rounded-3xl border border-slate-200/90 bg-white shadow-xs ${className}`}>
    {(title || actions) && (
...
export const Field: React.FC<{
  ...
  <div className={className}>
    <label htmlFor={htmlFor} className="mb-1 block text-[11px] font-bold uppercase tracking-wide text-slate-500">
      {label}
    </label>
    {children}
    {hint && !error && <p className="mt-1 text-[11px] leading-relaxed text-slate-400">{hint}</p>}
...
export const SelectField: React.FC<{
  ...
  <Field label={label} htmlFor={id} hint={hint}>
    <select
      ...
      value={value ?? ''}
      onChange={(event) => onChange(event.target.value)}
      className={`${inputClass} cursor-pointer ${value ? '' : 'text-slate-400'}`}
    >
...
export const NumberField: React.FC<{
  ...
  <Field label={label} htmlFor={id} hint={hint}>
    <div className="relative">
      ...
      />
      {suffix && (
        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-bold text-slate-400">{suffix}</span>
      )}
    </div>
  </Field>
);
...
export const SaveBar: React.FC<{
  ...
  <div className="sticky bottom-4 z-20 mt-5" aria-live="polite">
    ...
    >
      <p className={`text-xs font-bold ${message ? 'text-emerald-800' : dirty ? 'text-amber-800' : 'text-slate-500'}`}>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="rounded-xl px-3 py-2 text-xs font-bold text-slate-600 transition-colors hover:bg-slate-100 disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed"
        >
          Discard
        </button>
        <button
          ...
          onClick={onSave}
          disabled={!dirty || saving}
          className="flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-xs font-extrabold text-white shadow-sm transition-all hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
        >
