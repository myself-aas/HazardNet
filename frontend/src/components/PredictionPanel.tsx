...
const PredictionPanel: React.FC<PredictionPanelProps> = ({
  ...
  return (
    <div className="bg-white border border-carbon-20/90 text-carbon-80 rounded-3xl p-6 sm:p-8 shadow-md space-y-6 relative overflow-hidden transition-all duration-300">
      ...
      <div className="bg-carbon-05/90 border border-carbon-20/90 rounded-2xl p-6 relative overflow-hidden shadow-2xs hover:shadow-sm transition-all duration-300">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-[11px] uppercase font-mono font-extrabold text-carbon-60 tracking-wider">Primary Hazard Detected</span>
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-carbon-90 tracking-tight">{primary.name}</h2>
            </div>
          </div>

          <div className="flex items-center justify-between sm:justify-end gap-5 border-t sm:border-t-0 pt-4 sm:pt-0 border-carbon-20/80">
            <div className="text-left sm:text-right">
              <span className="text-xs text-carbon-60 block font-bold">Model Confidence</span>
              <span className="text-3xl sm:text-4xl font-black text-carbon-90 font-mono tracking-tight">
                {primaryConfidence}%
              </span>
            </div>
            ...
            <button
              ...
              className="px-5 py-3 bg-carbon-90 hover:bg-carbon-80 text-white font-extrabold rounded-2xl text-xs transition-all duration-200 shadow-md active:scale-98 flex items-center gap-2 min-h-[48px] cursor-pointer"
        ...
        <p className="text-xs sm:text-sm text-carbon-70 mt-4 pt-4 border-t border-carbon-20/80 leading-relaxed font-normal">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-carbon-05/90 border border-carbon-20/90 rounded-2xl p-5 space-y-3 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono font-extrabold text-carbon-70 uppercase tracking-wider">Physical Severity</span>
          ...
          <div className="w-full bg-carbon-20 h-4 rounded-full overflow-hidden p-0.5 border border-carbon-30/80">
            <div
          ...
          <div className="flex justify-between text-[11px] text-carbon-60 font-mono font-semibold">
            <span>0.00 (Low)</span>
            <span>0.50 (Mod)</span>
        ...
        <div className="bg-carbon-05/90 border border-carbon-20/90 rounded-2xl p-5 flex flex-col justify-between space-y-3 shadow-2xs">
          <div className="flex items-center justify-between text-xs font-mono">
            <span className="font-extrabold text-carbon-70 uppercase tracking-wider">Inference Speed</span>
            ...
            <span className="text-carbon-black font-extrabold bg-white px-3 py-1 rounded-full text-xs border border-carbon-20 shadow-2xs">
              {processingTimeMs > 0 ? `${processingTimeMs} ms` : 'not measured'}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2.5 text-xs font-mono">
            <div className="bg-white p-3 rounded-2xl border border-carbon-20/90 shadow-2xs space-y-0.5">
              <span className="text-[10px] text-carbon-60 block uppercase font-extrabold">Input Tensor</span>
              <span className="text-carbon-90 font-bold">(1,15,10,64,64)</span>
            </div>
            <div className="bg-white p-3 rounded-2xl border border-carbon-20/90 shadow-2xs space-y-0.5">
              <span className="text-[10px] text-carbon-60 block uppercase font-extrabold">Execution Engine</span>
              <span className="text-carbon-90 font-bold">TFLite WASM</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-carbon-05/90 border border-carbon-20/90 rounded-2xl p-5 space-y-3 shadow-2xs">
        <h3 className="text-xs font-extrabold text-carbon-80 uppercase tracking-wider font-mono">
        ...
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <XAxis type="number" domain={[0, 1]} tickFormatter={(v) => `${Math.round(v * 100)}%`} stroke="#77777a" fontSize={11} />
              <YAxis dataKey="name" type="category" stroke="#444447" fontSize={12} width={110} tickLine={false} />
              <Tooltip
                formatter={(val: number) => [`${(val * 100).toFixed(1)}%`, 'Probability']}
                contentStyle={{ backgroundColor: '#ffffff', borderColor: '#d1d1d1', borderRadius: '16px', color: '#17171b', fontSize: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}
              />
              <Bar dataKey="score" radius={[0, 8, 8, 0]}>
                {topResults.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={index === 0 ? '#17171b' : index === 1 ? '#58585b' : '#959599'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      {channelFeatures && (
        <div className="bg-carbon-05/90 border border-carbon-20/90 rounded-2xl p-5 space-y-3 shadow-2xs">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
            <div className="bg-white p-3.5 rounded-2xl border border-carbon-20/90 shadow-2xs space-y-0.5">
              <span className="text-[10px] text-carbon-60 block uppercase font-extrabold">NDVI (Vegetation)</span>
              <span className="text-emerald-700 font-black text-sm">{channelFeatures.ndvi ?? '0.42'}</span>
            </div>
            <div className="bg-white p-3.5 rounded-2xl border border-carbon-20/90 shadow-2xs space-y-0.5">
              <span className="text-[10px] text-carbon-60 block uppercase font-extrabold">NDWI (Water Index)</span>
              <span className="text-carbon-90 font-black text-sm">{channelFeatures.ndwi ?? '0.18'}</span>
            </div>
            <div className="bg-white p-3.5 rounded-2xl border border-carbon-20/90 shadow-2xs space-y-0.5">
              <span className="text-[10px] text-carbon-60 block uppercase font-extrabold">Precipitation Mean</span>
              <span className="text-carbon-90 font-black text-sm">{channelFeatures.precip_mean ?? '12.4'} mm</span>
            </div>
            <div className="bg-white p-3.5 rounded-2xl border border-carbon-20/90 shadow-2xs space-y-0.5">
              <span className="text-[10px] text-carbon-60 block uppercase font-extrabold">SAR VV Backscatter</span>
      ...
      <div className="bg-carbon-05/90 border border-carbon-20/90 rounded-2xl p-5 space-y-3 shadow-2xs">
        <div className="flex items-center gap-2 text-xs font-extrabold text-carbon-90 uppercase tracking-wider font-mono">
          <h3>Recommended Agricultural Action Plan</h3>
        </div>
        <ul className="space-y-2 text-xs sm:text-sm text-carbon-70 list-disc pl-5 leading-relaxed font-normal">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-3 border-t border-carbon-20">
        <span className="text-xs text-carbon-60 font-mono">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            disabled={isSaving}
            className="px-4 py-2.5 bg-nasa-red hover:bg-nasa-red-shade text-carbon-90 font-extrabold rounded-xl text-xs transition-all duration-200 flex items-center gap-2 min-h-[44px] shadow-xs disabled:opacity-50 active:scale-98"
          >
            {isSaving ? (
              <span>Saving...</span>
            ) : user ? (
          ...
          <button
            ...
            className="px-4 py-2.5 bg-white hover:bg-carbon-10 text-carbon-80 border border-carbon-20 rounded-xl text-xs font-bold transition-all duration-200 flex items-center gap-2 min-h-[44px] shadow-xs active:scale-98"
          >
