...
**HazardNet** publishes multi-hazard outlooks for Bangladesh agriculture. Batch forecasts are served as stored district-level results with producer dates and provenance. The web app does not run a CNN in the browser or perform inline model inference; cached data supports offline viewing, not new predictions.
...
3. **Stored Forecast Reads**: `POST /api/predict` reads the latest stored district/horizon result through Express or Vercel ([ADR 0009](docs/adr/0009-single-inference-path.md)). Missing per-class probabilities, driver values and model versions stay unknown rather than being synthesized. The batch FP32 model remains separate; the misnamed INT8 file is retired ([ADR 0007](docs/adr/0007-no-int8-bundle.md)).
...
9. **User Dashboards & Unique Profile URLs**: Every signed-in user gets a dedicated dashboard at `/dashboard` with a unique username that becomes their public profile URL (`/u/<username>`), ~40 Firestore-backed profile fields, avatar upload (client-side resize/compress with replace-on-update), connector integrations (Open-Meteo, WhatsApp, SMS, Slack, webhooks, …), and email/password sign-up with verification, plus Google and GitHub sign-in — see [docs/user-dashboard.md](docs/user-dashboard.md).
...
### Durable forecast ingestion

Successful ingestion now requires a committed Firestore Admin transaction. Configure server-only credentials and review transaction limits in [the stored-forecast runbook](docs/ops/2026-09-20-stored-forecasts.md). Missing credentials fail writes; they do not produce memory-only success. PostgreSQL forecast cutover ADR 0002 is superseded by ADR 0014.
