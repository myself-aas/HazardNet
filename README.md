...
4. **Automated MLOps Pipeline (Daily Produce → Hourly Refresh)**: A GitHub Actions workflow triggers the Kaggle Notebook daily to fetch the latest GEE satellite data, inject Open-Meteo forecasts, and run TFLite inference for all 64 districts; an **hourly** workflow then pulls the notebook's latest CSV output (`kaggle kernels output ashifahmedshuvo/hazardnet-auto-forecast-pipeline`), ingests it into the forecast store, and commits a refreshed static snapshot (`frontend/public/data/forecasts-latest.json`) so the entire website codebase redeploys with data at most one hour old. See [docs/ops/hourly_forecast.md](docs/ops/hourly_forecast.md).
