...
| # | Finding | Status |
| - | ------- | ------ |
| 1 | **Live surface serves no security headers** (`Access-Control-Allow-Origin: *`, no CSP/XCTO/XFO/Referrer/Permissions/COOP, HSTS without `includeSubDomains`) although `vercel.json` defines all of them, and `/.well-known/security.txt` 404s | Repo side fixed and made un-driftable (one canonical CSP module, parity test, `.well-known` cache rule, stricter SPA rewrite); **delivery is owner Action 7** (redeploy / check the Vercel root directory) |
...
| 5 | **Service worker cached credentialed responses**: `cache.put` ignores `Cache-Control: no-store`, so a duty officer's `Authorization: Bearer` read of unpublished alerts could be stored and replayed on a shared device | `isCacheableResponse()` in `frontend/src/serviceWorker.ts` (refuses credentialed requests, `no-store`/`private`, `Set-Cookie`, non-200/non-GET) wired into both network-response cache paths, pinned by `frontend/src/lib/__tests__/serviceWorkerCache.test.ts` |
