...
| Secret | Purpose |
|---|---|
| `EE_SERVICE_ACCOUNT_JSON` | Google Earth Engine service-account key |
| `HAZARDNET_API_URL` | Ingest endpoint (e.g. `https://hazardnet.live/api/v1/forecasts/update`) |
| `HAZARDNET_API_KEY` | Bearer token for the ingest endpoint |

### Output

A CSV pushed to `POST $HAZARDNET_API_URL` with `Authorization: Bearer $HAZARDNET_API_KEY` and `Content-Type: text/csv`. The server upserts rows into Firestore/Supabase, rebuilds the website snapshot, and invalidates caches.
...
1. Run `Actions → Verify GitHub Actions Secrets` (workflow added in `verify-secrets.yml`) to confirm all secrets are present.
2. Trigger `Actions → HazardNet Daily Forecast Pipeline → Run workflow` and confirm it completes in ~19 minutes.
3. Check `https://hazardnet.live/api/v1/forecasts/metadata` for a fresh `prediction_date`.
