...
*Delivered this phase:* the hindcast harness (`scripts/hindcast/`, four modules), five curated
episodes with sourced truth sets (Amphan 2020, Yaas 2021, Cyclone Mocha 2023, the 2024 eastern
floods, the 2025 northeast floods), five committed driver series, five CI-verified reports, a
producing workflow (`hindcast.yml`) and an offline `check` gate wired into `ci.yml`; 23 tests; and
the findings in `docs/phase-reports/phase-9-validation-and-soft-launch.md`:

* **Detection:** 50 of 50 named districts across five episodes reached the alarm list at both
  horizons; the class was named for 12 (all in the 2024 flood); for 0 of 27 on the three cyclones.
...
6. ~~**Publish the model-performance page**~~ — **done.** `/model-performance` is live in the build,
   generated by `scripts/build_model_performance.mjs` from the committed reports and gated by
...
**The 2023 episode (§8 item 9.7):** `data/hindcast/episodes/mocha-2023.json`, produced by
`hindcast.yml` in the bot commit `ef5888b` (which also regenerated and committed the derived
artifacts — the workflow now does that, because CI gates them).
