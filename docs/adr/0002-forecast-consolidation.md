...
- **Single access layer:** `backend/forecastStore.js` — every forecast
  ...
  `supabase`), fixed at first call; `getForecastStoreMode()` exposes the
...
- **Byte-compatible API:** the Supabase implementation coerces pg's string
  ...
  replace, Supabase `DISTINCT ON` query, delete+insert transaction with
...
- **Schema:** `scripts/db/002_forecasts_supabase.sql` extended to the notebook's
...
- **Migration:** `scripts/migrate-firestore-to-supabase.mjs` reads and writes
  the dual-track fields.
- **Fail-loud wiring:** `backend/server.js` logs a problems-level config error
  when `FORECAST_STORE=supabase` but `DATABASE_URL` is unset;
  `.github/workflows/weekly_forecast.yml` passes
  `FORECAST_STORE: ${{ vars.FORECAST_STORE || 'firestore' }}` +
  `DATABASE_URL: ${{ secrets.SUPABASE_DB_URL }}` (no-ops until activated).
- **Env documentation:** root `.env.example` and `frontend/.env.example`
  enumerate all runtime env vars incl. the cutover set.

## Ops runbook (migrate + flip — executing 2026-09-12; requires Supabase credentials)

**Credential protocol (least exposure):** the Supabase Postgres connection
...
as the repo secret `SUPABASE_DB_URL` via `gh secret set` (not persisted
locally). Rotate the database password in the Supabase dashboard afterwards
if desired.

0. **Hand-off:** owner provides the connection string; agent runs the steps.
1. **Apply the schema** — either the Supabase SQL editor with
   `scripts/db/002_forecasts_supabase.sql`, or the agent applies it over pg
   (idempotent — safe on a partially-migrated database).
2. **Verify the schema:** `DATABASE_URL=… node scripts/verify-supabase-cutover.mjs`
   ...
   the **Supabase cutover verify** workflow (`.github/workflows/
   supabase-cutover-verify.yml`, manual dispatch) runs the checklist against
   the `SUPABASE_DB_URL` secret **and** boots the backend in supabase mode
...
3. **Migrate** (Firestore is expected empty — the pipeline never successfully
   wrote): `node scripts/migrate-firestore-to-supabase.mjs` (dry-run), then
   `FIRESTORE_TO_SUPABASE_MIGRATION=RUN …` if rows exist. Legacy 7/15-era
   rows are skipped and reported, never migrated (ADR 0005 CHECK).
4. **Flip the flags:** `gh secret set SUPABASE_DB_URL` + `gh variable set
   FORECAST_STORE --body supabase` (workflow), and set `DATABASE_URL` +
   `FORECAST_STORE=supabase` in the Vercel project env (dashboard — no agent
   access) and any backend runtime.
5. **Live smoke** with `FORECAST_STORE=supabase node backend/server.js`:
...
- [x] Ingest writes land through the store in both modes (transaction/rollback
      and upsert paths unit-tested; supabase mode wired end-to-end — live smoke
...
- [ ] `FORECAST_STORE=supabase` + `DATABASE_URL` set in backend/Vercel/workflow;
      weekly pipeline writes land in Supabase and are visible to the frontend (ops)
