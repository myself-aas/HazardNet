...
**Status: IMPLEMENTED (Phase 4 backend, 2026-09-17; Phase 5 web surfaces, 2026-09-18).**
`REQUIRED_DISCLAIMER` in `backend/alerts/policy.js` is byte-compared against this block by
...
On the web, `frontend/src/lib/legal.ts::ALERT_DISCLAIMER` is the single copy rendered by
`/alerts`, `/alerts/:id`, the district strip and the printed evidence card;
`scripts/tests/test_frontend_alert_surface.py` asserts it equals the backend constant (and
the snapshot builder's copy), so the site, the API, the SMS gateway and the committed
offline snapshot cannot drift. The canonical text is marked `lang="en"` when the page is
Bengali so a screen reader pronounces it with an English voice.
...
| 2026-09-18 | 1.5-draft | Phase 5 (frontend). New public surface: `/alerts` (level/hazard/horizon/evidence/policy, filterable, with a district table as the map's text alternative), `/alerts/:id` (printable evidence card, PDF export through the shared exporter, CSV export with a UTF-8 BOM), and an alert strip on `/district/:id`. Every surface labels its data source — live API, service-worker offline copy, committed snapshot (schema `hazardnet-alerts/v1`), or unavailable — and reports dropped non-`PUBLISHED` rows and payload warnings instead of hiding them. The UI is bilingual (Bengali numerals, long-form dates, per-element `lang`, hazard-class names mapped to the model's eight classes); low-bandwidth mode (Data Saver, 2G, ≤2 GB RAM, ≤4 cores, offline, or the user's toggle) substitutes the vector basemap for satellite raster tiles, collapses animation in CSS and opens on the text table. Accessibility: axe-core clean on the new components in both languages (17 assertions, one real landmark defect found and fixed) — see `docs/frontend/ACCESSIBILITY.md`, which also lists what the automated pass cannot cover. §5.2: the README's 10/20/30-day horizons and 507-unit ADM3 pitch were still presented as shipped; they are corrected to the delivered 64 districts × 7/15 days with ADR 0005 marked pending, and a new guard (`test_readme_does_not_advertise_unshipped_horizons`) fails the build if the README advertises them again. Alert thresholds remain placeholders pending Phase 9, `WARNING` remains unreachable from model evidence, and the committed alert snapshot is empty with all 74 rows `publication_blocked` (§1.6 requires a model version the pipeline does not yet stamp). |
| 2026-09-18 | 1.6-draft | Phase 6 (security). The audit ran against the deployed surface rather than the repository, and the two disagreed: `www.hazardnet.live` served none of the security headers `vercel.json` declares (no CSP, XCTO, XFO, Referrer-Policy, Permissions-Policy or COOP) and HSTS without `includeSubDomains`, because production appears to deploy `frontend/` as its Vercel root (the API handlers and the root config 404 live while `frontend/public/` assets serve) and to track a `main` that is behind this branch. In-repo the policy now has one definition (`backend/security/csp.js`) that both Vercel configs and helmet consume, with a parity test, and delivery is owner Action 7. Also fixed: the serverless suite had **no rate limiting** (the Express limiters do not run on Vercel) and no hardening headers — `backend/middleware/serverlessGuard.js` now applies both to all 13 handlers, per-instance and says so; both CSV exports could execute a formula in Excel (`=HYPERLINK…`/DDE) and now neutralise leading `=`/`+`/`-`/`@`/tab/CR while leaving numbers intact; the service worker cached a duty officer's credentialed `Authorization` response despite `no-store` (`isCacheableResponse()`); the chat prompt's per-field bounds were computed for retrieval but the model prompt was built from the raw body — a 50 kB query reached the model, and the assembled prompt is now capped as a whole; `firestore.rules` read camelCase ownership while the client writes `user_id`, so owners were denied their own assessment/alert/connector rows and the profile validator never ran (both spellings accepted, validators applied). The 1.5 offline story is corrected: the service-worker alert cache had been written into an unimported duplicate (`frontend/src/serviceWorker.ts`) while the browser registers `frontend/public/serviceWorker.js`, so it never shipped — it now lives in the shipped file, the duplicate is deleted, and a test drives the shipped worker's real fetch handler. The disclosure surface is closed: `SECURITY.md` plus a validity/parity-tested RFC 9116 `security.txt`. One critical process finding: `scripts/check-secrets.sh` matched its placeholder allowlist against the whole `file:line:match`, so the token `EXAMPLE` in the path `.env.example` suppressed every hit inside it — live provider keys sat in that tracked file while CI printed "passed". The scanner is fixed, regression-tested and the template is placeholder-only; **rotation of the exposed credentials is owner Action 1 and no repository change can substitute for it** (`docs/audits/2026-09-18-secret-scan-false-negative.md`). `eslint .` (CI's 0-error gate) also failed on a clean checkout and is fixed. Nothing in this phase changes the product contract; alert thresholds remain Phase 9 work. |
| 2026-09-18 | 1.7-draft | Phase 7 (observability). §1.4 cadence and §5.1 coverage are now published rather than implied: `scripts/build_freshness_artifact.mjs` derives `frontend/public/data/freshness.json` (`hazardnet-freshness/v1`) from the committed ingest manifest, website snapshot, alert snapshot and probe result, and `/status` renders it — as static HTML at build time and as a React panel at runtime, from the same file. Each source carries a state (`Within SLO` / `Past SLO` / `Checks failing` / `No data` / `Unknown`), its age, the SLO it is measured against and a reason, so a reader can check any claim against a named file. The artifact copies the coverage stamp and the model provenance verbatim: while the pipeline stamps no `model_version`, the page says **Not stamped** and states why §1.6 keeps publication closed above `WATCH`. The site-health probe publishes a machine-readable result (`data/site-health/latest.json`, `hazardnet-site-probe/v1`) that the artifact quotes, and now checks the status page and its artifact against the data the deployment actually serves. The page deliberately claims no uptime and no liveness (it is a statement about committed files, not a probe of the running services), names no missing districts the run did not name, and is English-only — the bilingual surface remains the alert UI. `node scripts/build_freshness_artifact.mjs --check` gates drift between the artifact and its inputs (clock-blind), `scripts/tests/test_status_surface.py` re-derives the numbers independently, and `__tests__/freshnessArtifact.test.js` / `frontend/src/pages/__tests__/StatusPage.test.tsx` pin the states, the unknown-age rendering and the axe-clean panel. Thresholds remain Phase 9 work. |
| 2026-09-18 | 1.8-draft | Phase 8 (SEO and the content engine). The site now publishes the model's own numbers per district instead of only describing the system: `scripts/build_content_engine.mjs` composes `/hazards` (index) + one methodology page per hazard class + `/districts` (index) + one outlook page per district — and the season retrospectives when an event archive is loaded — from the committed forecast snapshot, the authored prose in `src/content/hazard-methodology.json` and the app's own 64-district table, and `frontend/scripts/prerender.mjs` writes every one of them to static HTML with its own title, canonical, robots directive, breadcrumb trail and JSON-LD. Every hazard page states what the model labels, the formula its independent physics cross-check computes, the confidence semantics (`uncalibrated_model_softmax`) and what the class cannot tell you; a district page shows the current run's hazard class, severity, confidence and divergence per horizon, the static baseline the map falls back to, and — because the 2,931-event archive of §4 is not in this repository — says plainly that no archive is loaded rather than printing a history count it did not read. The four districts the committed run does not cover are `noindex,follow` and stay out of the sitemap. Structured data (`WebSite`, `Organization`, `Person` ×3 from `src/content/attribution.json`, `WebPage`, `SoftwareApplication`, `Dataset` for the forecast snapshot, `FAQPage`, `BreadcrumbList`) is built by one module shared by the prerenderer and the SPA; the historical-archive `Dataset` node appears only when an archive was actually loaded and carries no licence or distribution it cannot back up. The canonical host is now declared in the repository as `www.hazardnet.live` — both `vercel.json` copies redirect the apex, the build fails on any other origin, and the sitemap/robots/canonicals all agree. `CITATION.cff` makes the repository citable. Source reconciliation: the snapshot still spells eight districts differently from the app's table (`Chittagong`, `Jessore`, `Comilla`, `Nawabganj`, …) and the engine maps them explicitly, reporting anything it cannot match instead of guessing. Discoverability is owner Action 10 (Search Console/Bing), citations are Action 11, and publishing the archive is Action 12; nothing in the changelog claims any of them happened. Alert thresholds remain Phase 9 work. |
